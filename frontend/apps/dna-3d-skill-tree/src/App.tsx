      ...
      >
        <div
          style={{
            ...
            transform: repo.strand === "practice"
              ? "translateX(28px)"
              : repo.strand === "theory"
              ? "translateX(-28px)"
              : "translateY(-32px)",
...
function CameraRig({ selected }) {
  ...
  useEffect(() => {
    if (selected) {
      ...
      const pos = selected.strand === "center"
...
function Panel({ selected, onClose }) {
  ...
  const strandLabel = { theory: "THEORY STRAND", practice: "PRACTICE STRAND", center: "ARCHITECTURE CORE" }[selected.strand];
    ...
    >
      ...
      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 9, letterSpacing: "0.25em", color, marginBottom: 6 }}>{strandLabel}</div>
